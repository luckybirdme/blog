...
- 线程：CPU 调度执行的最小单位，不能独立存在，依赖进程存在，一个进程至少有一个线程，叫主线程，而多个线程共享进程的大部分资源，线程切换比进程代价低，从而提高了程序的运行效率，但是一个进程的线程无法在多个 CPU 运行。
...
- 缺点
 ...
 - 进程通信借助 IPC(管道、消息队列、信号量、共享存储、Socket等)，成本较高，大数据传输也相对较慢
