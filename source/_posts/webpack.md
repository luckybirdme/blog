...
- 获取参数：从配置文件和 Shell 语句中读取与合并参数，得出最终的参数；
- 初始化：用上一步得到的参数进行初始化，加载所有配置的插件，执行对象的 run 方法开始执行编译；
- 确定入口文件：根据配置中的 entry 找出所有的入口文件；
- 开始编译模块：从入口文件出发，调用所有配置的 Loader 对各个模块进行翻译，比如非 JS 的CSS，再找出该模块依赖的模块，再递归本步骤直到所有入口依赖的文件都经过了本步骤的处理；
...
- 新增了mode参数来表示是开发还是生产，production 侧重于打包后的文件大小，development侧重于构建树。
